...
def Efs_field_sin2(t):
    ...
    result[idx] = E1*np.sin(w*(t[idx]-duration/2)/(2.*nc))**2*np.cos(w*t[idx])
    return result
...
def Eds_field_sin2(t, DT):
    ...
    result[idx] = E2* np.sin(w*(t[idx]-duration/2-DT*41.34)/(2.*nc))**2*np.cos(2*w*(t[idx]-DT*41.34))
    return result
...
tau0 = np.linspace(-15., 15., 420)[280:420]
...
np.savetxt('dipole_d_S3_term12.dat', np.transpose(data_S))
np.savetxt('dipole_d_P3_term12.dat', np.transpose(data_P))
